...
int psp_fmgr_getExtId(const char *szFilePath)
{
  char *pszExt;
...
int psp_fmgr_get_dir_list(char *basedir, int dirmax, char **dirname)
{
  int index = 0;
  DIR *fd = 0;
  nfiles = 0;
...
int psp_file_find_first(char first_char, int check_last)
{
...
int psp_file_request(char *out, char *pszStartPath)
{
  static int sel = 0;
  ...
  for (;;)
  {
    ...
    else if ((new_pad & GP2X_CTRL_CROSS) || (new_pad & GP2X_CTRL_CIRCLE))
    {
      if (sortfiles[sel]->d_type == DT_DIR)
      {
        check_last = 0;
...
int psp_fmgr_menu(int format)
{
  ...
  if (format == FMGR_FORMAT_KBD)
    user_filedir = user_filedir_kbd;
  else if (format == FMGR_FORMAT_JOY)
    user_filedir = user_filedir_joy;
  else if (format == FMGR_FORMAT_SET)
    user_filedir = user_filedir_set;
  else if (format == FMGR_FORMAT_CHT)
    user_filedir = user_filedir_cht;
  else
    user_filedir = user_filedir_rom;
  ...
  if (psp_file_request(user_filename, user_filedir))
  {
    ...
    else
    {
      ...
      if (file_format == FMGR_FORMAT_ZIP)
      {
        ...
        if (user_file_format == FMGR_FORMAT_ROM) /* Rom */
          error = atari_load_rom(user_filename, 1);
      }
      else
      {
        if (file_format == FMGR_FORMAT_ROM) /* Rom */
          error = atari_load_rom(user_filename, 0);
        else if (file_format == FMGR_FORMAT_KBD) /* Kbd */
          error = psp_kbd_load_mapping(user_filename);
        else if (file_format == FMGR_FORMAT_JOY) /* Joy */
          error = psp_joy_load_settings(user_filename);
        else if (file_format == FMGR_FORMAT_SET) /* Settings */
          error = atari_load_file_settings(user_filename);
        else if (file_format == FMGR_FORMAT_CHT) /* Cheat */
          error = atari_load_file_cheat(user_filename);
      }
    }
